...
function fetchAndRenderSupportTickets() {
  ...
  fetch(`http://127.0.0.1:8000/api/v1/shopify/handler/shop?${query}`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  })
...
async function closeSupportTicket(ticketId) {
  ...
  try {
    const response = await fetch("http://127.0.0.1:8000/api/v1/shopify/handler/shop/close", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: ticketId,
...
function createSupportTicket() {
  ...
  fetch("http://127.0.0.1:8000/api/v1/shopify/handler/shop", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  })
...
function getSupportTicketOrderOptions() {
  const orderSources = [window.customerOrders, window.ordersData, typeof customerOrders !== "undefined" ? customerOrders : null];
  const orders = orderSources.find((source) => Array.isArray(source)) || [];

  return orders
    .map((order) => {
      const value = order?.id || order?.orderId || order?.order_id || order?.orderNumber || order?.name;
...
function attachTicketOrderSelectLogic() {
  ...
  const setOrder = (value, selectedLabel) => {
    ...
    if (!value || !selectedLabel) {
      label.textContent = "Select an order";
