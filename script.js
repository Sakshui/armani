...
function attachTicketConversationModalLogic() {
  ...
  container.addEventListener("click", (event) => {
    ...
    const ticket = (window.supportTicketState?.tickets || [])
      .find(t => String(t.id) === String(ticketId));
    ...
    // Populate text
    setText("conversationTitle", normalized.title);
    setText("conversationTicketId", normalized.meta.split("·")[0]?.trim());
    setText("conversationType", normalized.tag);
    setText("conversationCreatedAt", normalized.date);
    setText("conversationCustomerDate", normalized.date);
    setText("conversationCustomerMessage", normalized.desc);
    ...
    if (statusBadge) {
      ...
      statusBadge.textContent = normalized.status;
      statusBadge.style.cssText =
        `display:inline-flex;padding:3px 12px;border-radius:999px;font-size:15px;font-weight:600;${statusStyles[key] || ""}`;
    }

    // Priority badge
    const priorityBadge = document.getElementById("conversationPriority");
    if (priorityBadge) {
      const key = normalized.priority.toLowerCase();
      priorityBadge.textContent = normalized.priority;
      priorityBadge.style.cssText =
        `display:inline-flex;padding:3px 12px;border-radius:999px;font-size:15px;font-weight:600;${priorityStyles[key] || ""}`;
    }
...
async function getSupportTicketTaxonomy(webUrl) {
  ...
  try {
    const taxonomyPath = "/api/v1/shopify/handler/outlet/ticket-taxonomy/";
...
async function getSupportTicketTaxonomyData(webUrl) {
  if (!webUrl) return [];
  ...
  try {
    ...
    return Array.isArray(data?.data?.ticket_taxonomy)
      ? data.data.ticket_taxonomy
      : [];
  } catch (error) {
    console.warn("⚠️ Failed to fetch ticket taxonomy data.", error);
    return [];
  }
}
...
async function loadIssueTypes() {
  ...
  try {
    ...
    const url = `${getSupportApiUrl(taxonomyPath)}?${query}`;
...
async function createSupportTicket() {
  const issueInput = document.getElementById("ticketIssueInput");
  const categoryInput = document.getElementById("ticketCategoryInput");
  const subCategoryInput = document.getElementById("ticketSubCategoryInput");
  ...
  const issueId = Number(issueInput?.value || 0);
  const categoryId = Number(categoryInput?.value || 0);
  const subCategoryId = Number(subCategoryInput?.value || 0);

  if (!issueId || !categoryId || !subCategoryId || !subject || !description) {
    createStatusToast("Missing Fields", "Issue type, category, sub category, subject and description are required.", "error");
    return;
  }

  const selectedIssueName = issueInput?.selectedOptions?.[0]?.textContent?.trim() || "General";
...
function attachTicketTaxonomySelectLogic() {
  const issueInput = document.getElementById("ticketIssueInput");
  const categoryInput = document.getElementById("ticketCategoryInput");
  const subCategoryInput = document.getElementById("ticketSubCategoryInput");

  if (!issueInput || !categoryInput || !subCategoryInput) return;

  const { web_url } = getSupportTicketRequestContext();

  const renderOptions = (selectEl, options, placeholder, idKey, nameKey) => {
    const optionRows = [`<option value="">${placeholder}</option>`];
    options.forEach((option) => {
      optionRows.push(`<option value="${option[idKey]}">${escapeHtml(option[nameKey])}</option>`);
    });
    selectEl.innerHTML = optionRows.join("");
  };


  const toggleDisabled = (selectEl, shouldDisable) => {
    selectEl.disabled = shouldDisable;
    selectEl.style.opacity = shouldDisable ? "0.6" : "1";
    selectEl.style.cursor = shouldDisable ? "not-allowed" : "pointer";
  };


  const resetCategory = () => {
    renderOptions(categoryInput, [], "Select category", "category_id", "category_name");
    toggleDisabled(categoryInput, true);
  };

  const resetSubCategory = () => {
    renderOptions(subCategoryInput, [], "Select sub category", "sub_category_id", "sub_category_name");
    toggleDisabled(subCategoryInput, true);
  };

  resetCategory();
  resetSubCategory();

  getSupportTicketTaxonomyData(web_url).then((taxonomyData) => {
    console.log("TAXONOMY DATA:", taxonomyData);
    if (!Array.isArray(taxonomyData) || taxonomyData.length === 0) {
      return;
    }

    renderOptions(issueInput, taxonomyData, "Select issue type", "issue_id", "issue_name");

    issueInput.onchange = () => {
      const selectedIssueId = Number(issueInput.value || 0);
      const selectedIssue = taxonomyData.find((item) => Number(item.issue_id) === selectedIssueId);

      resetSubCategory();

      if (!selectedIssue || !Array.isArray(selectedIssue.categories) || selectedIssue.categories.length === 0) {
        resetCategory();
        return;
      }

      renderOptions(categoryInput, selectedIssue.categories, "Select category", "category_id", "category_name");
      toggleDisabled(categoryInput, false);
    };

    categoryInput.onchange = () => {
      const selectedIssueId = Number(issueInput.value || 0);
      const selectedCategoryId = Number(categoryInput.value || 0);

      const selectedIssue = taxonomyData.find((item) => Number(item.issue_id) === selectedIssueId);
      const selectedCategory = selectedIssue?.categories?.find((item) => Number(item.category_id) === selectedCategoryId);

      if (!selectedCategory || !Array.isArray(selectedCategory.sub_categories) || selectedCategory.sub_categories.length === 0) {
        resetSubCategory();
        return;
      }

      renderOptions(subCategoryInput, selectedCategory.sub_categories, "Select sub category", "sub_category_id", "sub_category_name");
      toggleDisabled(subCategoryInput, false);
    };
  });
}
...
function attachModalLogic() {
  ...
  attachTicketTaxonomySelectLogic();
  loadIssueTypes();
